...
class BusinessSettingsController extends BaseController
{
    ...
    public function updateOrderWiseShippingCost(Request $request): RedirectResponse
    {  
        $request->validate([
            'order_wise_shipping_cost' => 'required|numeric|min:0',
        ]);

        $this->businessSettingRepo->updateOrInsert(type: 'order_wise_shipping_cost',value: $request->order_wise_shipping_cost);

        clearWebConfigCacheKeys(); 

        ToastMagic::success(translate('Order_wise_shipping_cost_updated_successfully'));
        return back();
    }
