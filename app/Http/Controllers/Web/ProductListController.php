    ...
    public function default_theme($request): View|JsonResponse|Redirector|RedirectResponse
    {
        ...
        if ($request['data_from'] == 'category') {
            $pageTitleContent = Category::when($request['category_id'], function ($query) use ($request) {
                ...
                return $query->where('id', $request['sub_sub_category_id']);
            })->first()?->name ?? translate('Category');
        }
        if ($request['data_from'] == 'brand') {
        ...
        if ($request->has('publishing_house_id')) {
            $pageTitleContent = PublishingHouse::firstWhere('id', $request['publishing_house_id'])?->name;
        }
