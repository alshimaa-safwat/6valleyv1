...
class CartManager
{
    ...
    public static function getCartListQueryAPI($request = null, $groupId = null, $type = null)
    {
    ...
    public static function updateOrderSummaryShippingCost($groupId = null, $type = null): void
    {
        ...
        self::updateProductShippingCost($cartItems,($type == 'checked'));
    }

    public static function updateProductShippingCost($cartList,$onlyChecked = false): void
    {
        ...
        foreach($cartList as $cartKey => $cartItem) {
            ...
            Cart::where(['id' =>  $cartItem->id])->update([
             ...
             if ($shippingType == 'order_wise') {
                $existingShipping = CartShipping::where('cart_group_id', $cartItem->cart_group_id)->first();
                if ($existingShipping) {
                    $method = ShippingMethod::find($existingShipping->shipping_method_id);
                    if ($method) {
                        // $totalWeight = self::getCartTotalWeight($cartItem->cart_group_id);
                        $totalWeight = self::getCartTotalWeight($cartItem->cart_group_id, $onlyChecked);
                        // $orderWiseCost = getWebConfig('order_wise_shipping_cost') ?? 0;
                        $existingShipping->shipping_cost = $totalWeight * ($method->cost ?? 0); ;
        ...
        if ($request['buy_now'] == 1) {
            ...
            if ($product['product_type'] == 'physical' && $shippingType == 'order_wise') {
                if ($request['shipping_method_exist'] && $request['shipping_method_id'] && count($sellerShippingList) > 0) {
                    ...
                    // $totalWeight = self::getCartTotalWeight($cart['cart_group_id']);
                    $totalWeight = self::getCartTotalWeight($cart['cart_group_id'], true);
                    $shipping['shipping_cost'] = ($totalWeight * ($getShippingCost->cost ?? 0));
                    ...
                    return [
                        ...
                        'cart_shipping_cost' => $shipping['shipping_cost'] ?? 0,
                        'message' => translate('successfully_added') . '!',
                    ];
                }
                return [
    ...
    //     public static function getCartTotalWeight($cartGroupId)
    // {
    //     $totalWeight = 0;
    //     $cartItems = Cart::where('cart_group_id', $cartGroupId)
    //         ->whereHas('product', fn($q) => $q->active())
    //         ->get();

    //     foreach ($cartItems as $item) {
    //         $weightPerUnit = $item->product->weight ?? 0;
    ...
    // }
        ...
        public static function getCartTotalWeight($cartGroupId, $onlyChecked = false)
    {
        $totalWeight = 0;
        $cartItems = Cart::where('cart_group_id', $cartGroupId)
            ->whereHas('product', fn($q) => $q->active())
            ->when($onlyChecked, function($query) {
                return $query->where('is_checked', 1);
            })
            ->get();
