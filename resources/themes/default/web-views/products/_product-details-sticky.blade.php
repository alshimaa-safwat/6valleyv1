<div
    class="bg-white product-details-sticky product-details-sticky-section pt-4 pt-md-3 pb-3 {{ $productDetails->variation && count(json_decode($productDetails->variation)) > 0 ? 'multi-variation-product' : '' }}">
    <div class="btn-circle product-details-sticky-collapse-btn d-md-none transition cursor-pointer shadow-sm position-absolute translate-middle top-0 left-50 justify-content-center align-items-center {{ $productDetails->variation && count(json_decode($productDetails->variation)) > 0 ? 'd-flex' : 'd-none' }}"
        style="--size: 34px">
        ...
        <form class="add-to-cart-sticky-form addToCartDynamicForm">
            ...
            <div class="product-details-sticky-top">
                <div class="border-bottom d-flex flex-column gap-3 mb-3 pb-3">
                    ...
                    @php
                        $stickyProductShapes = json_decode($productDetails->shapes) ?? [];
                    @endphp
                    @if (count($stickyProductShapes) > 0)
                        <div class="position-relative">
                            <h6 class="fs-14 mb-2 pb-2">
                                {{ translate('shape') }}
                                <span class="text-muted font-weight-light product-details-sticky-shape-name"></span>
                            </h6>
                            <div>
                                <ul
                                    class="list-inline checkbox-alphanumeric checkbox-alphanumeric--style-1 m-0 px-0 flex-start gap-2 overflow-x-auto flex-nowrap overflow-y-hidden scrollbar-none">
                                    @foreach ($stickyProductShapes as $key => $shapeId)
                                        @php
                                            $stickyShape = \App\Models\Shape::find($shapeId);
                                        @endphp
                                        @if ($stickyShape)
                                            <li class="user-select-none">
                                                <input type="radio"
                                                    id="sticky-{{ str_replace(' ', '', $productDetails->id . '-shape-' . $shapeId) }}"
                                                    name="shape" value="{{ $shapeId }}"
                                                    @if ($key == 0) checked @endif>
                                                <label class="__text-12px focus-preview-image-by-shape m-0"
                                                    for="sticky-{{ str_replace(' ', '', $productDetails->id . '-shape-' . $shapeId) }}"
                                                    data-toggle="tooltip" data-key="{{ $shapeId }}"
                                                    data-shapeid="preview-box-shape-{{ $shapeId }}"
                                                    data-title="{{ $stickyShape->name }}">
                                                    <span class="text-nowrap">{{ $stickyShape->name }}</span>
                                                </label>
                                            </li>
                                        @endif
                                    @endforeach
                    ...
                    @if (
                        $productDetails['product_type'] == 'digital' &&
                            $productDetails['digital_product_file_types'] &&
                            count($productDetails['digital_product_file_types']) > 0 &&
                            $productDetails['digital_product_extensions']
                    )
                        @foreach ($productDetails['digital_product_extensions'] as $extensionKey => $extensionGroup)
                            <div>
                                ...
                                @if (count($extensionGroup) > 0)
                                    <div
                                        class="list-inline checkbox-alphanumeric checkbox-alphanumeric--style-1 mb-0 flex-start row ps-0 overflow-x-auto flex-nowrap overflow-y-hidden scrollbar-none">
                                        @foreach ($extensionGroup as $index => $extension)
                                            <div class="user-select-none">
                                                <div class="for-mobile-capacity">
                                                    ...
                                                    <label
                                                        for="sticky-extension_{{ str_replace(' ', '-', $extension) }}"
                                                        class="__text-12px">
                                                        <span
                    ...
                    @foreach (json_decode($productDetails->choice_options) as $key => $choice)
                        <div>
                            ...
                            <div
                                class="list-inline checkbox-alphanumeric checkbox-alphanumeric--style-1 mb-0 flex-start ps-0 overflow-x-auto flex-nowrap overflow-y-hidden scrollbar-none">
                ...
                <div class="media gap-sm-3 d-flex flex-column flex-sm-row">
                    ...
                    <div class="media-body">
                        ...
                        <div class="d-flex flex-wrap align-items-center mb-2 pro">
                            <span
                                class="fs-12 text-muted line--limit-1 text-capitalize product-generated-variation-text"></span>
                            <div class="d-none d-sm-flex flex-wrap align-items-center">
                                <span
                                    class="{{ count(json_decode($productDetails->variation, true)) > 0 ? '__inline-25' : '' }} {{ count(json_decode($productDetails->variation, true)) > 0 ? 'mx-2' : '' }} mt-0"></span>
                                ...
                                <span
                                    class="for-discount-value position-static p-1 px-2 font-bold fs-13 mx-2 discounted-badge-element">
                ...
                <div class="d-flex align-items-center gap-2 gap-sm-3 gap-xl-4">
                    <div
                        ...
                        <span class="input-group-btn h-100">
                            <button
                                class="btn btn-number __p-10 web-text-primary bg-ECF1F6 rounded-0 h-100 w-32px d-flex align-items-center"
                                type="button" data-type="minus" data-field="quantity" disabled="disabled">-</button>
                        </span>
                        ...
                        <span class="input-group-btn h-100">
                            <button
                                class="btn btn-number __p-10 web-text-primary bg-ECF1F6 rounded-0 h-100 w-32px d-flex align-items-center"
                                type="button" data-producttype="physical" data-type="plus"
                    ...
                    <div class="font-weight-normal text-accent align-items-end gap-2 d-none d-lg-flex">
                        <span
                            class="product-bottom-section-price fs-24 font-bold user-select-none text-nowrap">{{ $initialProductConfig['price'] }}</span>
                    </div>


                    @if (
                        ($product->added_by == 'admin' &&
                            (checkVendorAbility(type: 'inhouse', status: 'temporary_close') ||
                                checkVendorAbility(type: 'inhouse', status: 'vacation_status'))) ||
                            ($product->added_by == 'seller' &&
                                (checkVendorAbility(type: 'vendor', status: 'temporary_close', vendor: $product->seller->shop) ||
                    ...
                    @else
                        <div class="product-add-and-buy-section d-flex gap-2">
                            ...
                            <button class="btn btn--primary element-center product-add-to-cart-button" type="button"
                                data-form=".add-to-cart-sticky-form" data-update="{{ translate('update_cart') }}"
                        ...
                        @if ($productDetails['product_type'] == 'physical')
                            <div class="product-restock-request-section collapse" {!! $firstVariationQuantity <= 0 ? 'style="display: block;"' : '' !!}>
                                <button type="button"
                                    ...
                                    data-auth="{{ auth('customer')->check() }}" data-form=".add-to-cart-sticky-form"
