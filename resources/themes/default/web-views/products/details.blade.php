                                        ...
                                        <div class="swiper-container quickviewSlider2 border rounded aspect-1">
                                            <div class="swiper-wrapper">
                                                @php
                                                    ...
                                                    $shapeImageSources = $product->product_type === 'physical' &&
                                    ...
                                    <div class="details __h-100 product-cart-option-container p-0">
                                        ...
                                        <form
                                            ...
                                            <div class="position-relative">
                                                ...
                                                @endif
                                                @php
                                                    $productShapes = json_decode($product->shapes) ?? [];
                                                @endphp
                                                @if (count($productShapes) > 0)
                                                    <div class="d-flex align-items-center gap-3 my-2">
                                                        <div class="product-description-label m-0 text-dark">
                                                            {{ translate('shape') }}
                                                        </div>
                                                        <div>
                                                            <ul class="list-inline checkbox-alphanumeric checkbox-alphanumeric--style-1 mb-0 p-0 flex-start gap-2">
                                                                @foreach ($productShapes as $key => $shapeId)
                                                                    @php
                                                                        $shape = \App\Models\Shape::find($shapeId);
                                                                    @endphp
                                                                    @if($shape)
                                                                        <li>
                                                                            <input type="radio"
                                                                                id="{{ str_replace(' ', '', $product->id . '-shape-' . $shapeId) }}"
                                                                                name="shape" value="{{ $shapeId }}"
                                                                                @if ($key == 0) checked @endif>
                                                                            <label class="__text-12px focus-preview-image-by-shape"
                                                                                for="{{ str_replace(' ', '', $product->id . '-shape-' . $shapeId) }}"
                                                                                data-toggle="tooltip"
                                                                                data-key="{{ $shapeId }}"
                                                                                data-shapeid="preview-box-shape-{{ $shapeId }}"
                                                                                data-title="{{ $shape->name }}">
                                                                                <span class="text-nowrap">{{ $shape->name }}</span>
                                                                            </label>
                                                                        </li>
                                                                    @endif
                                                                @endforeach
