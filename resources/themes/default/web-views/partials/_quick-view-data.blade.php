                ...
                <div class="swiper-container quickviewSlider2 border rounded aspect-1">
                    <div class="swiper-wrapper">
                        @php
                            ...
                            $shapeImageSources =
                                $product->product_type === 'physical' &&
                                !empty($product->shape_image) &&
                                count($product->shape_images_full_url) > 0
                                    ? $product->shape_images_full_url
                                    : [];
                        @endphp
                        ...
                        @foreach ($shapeImageSources as $key => $photo)
                            @php
                                $imagePath = isset($photo['image_name'])
                                    ? getStorageImages(path: $photo['image_name'], type: 'backend-product')
                                    : getStorageImages(path: $photo, type: 'backend-product');
                                ...
                                $shapeId = $photo['shape'] ?? '';
                            @endphp
                            <div class="swiper-slide position-relative" data-shape="{{ $shapeId }}">
                                <div class="easyzoom easyzoom--overlay is-ready">
                                    <a href="{{ $imagePath }}">
                                        <img class="rounded h-100 aspect-1" alt="" src="{{ $imagePath }}">
                                    </a>
                ...
                <div class="cz-product-gallery-icons">
                    <div class="d-flex flex-column gap-12px pt-3">
                        @if ($product->product_type == 'physical')
                            <div class="bg-white btn-circle border" style="--size: 35px" data-toggle="tooltip"
                        ...
                        <div class="share_dropdown_wrapper">
                            <button type="button" class="btn btn-outline-primary btn-circle p-0 share_btn"
                            ...
                            <div class="share_dropdown bg-white d-flex gap-3 align-items-center flex-column">
                                <a href="#"
                                    ...
                                    style="--size: 20px" data-action="{{ route('product', $product->slug) }}"
            ...
            <div class="details __h-100 product-cart-option-container border-0 py-0">
                @if ($product->added_by == 'admin')
                    <a href="{{ route('shopView', ['slug' => getInHouseShopConfig(key: 'slug')]) }}"
                        class="d-block pb-2 text-truncate">{{ getInHouseShopConfig('name') ?? '' }}</a>
                @else
                    <a href="{{ route('shopView', ['slug' => $product->seller?->shop?->slug]) }}"
                        class="d-block pb-2 text-truncate">{{ $product->seller?->shop?->name ?? '' }}</a>
                @endif
                <a href="{{ route('product', $product->slug) }}"
                    class="fs-18 fw-bold text-title mb-3">{{ $product->name }}</a>
                <div class="d-flex flex-wrap align-items-baseline gap-3 mb-3 pro">
                    ...
                    <span
                        ...
                        <span class="web-text-primary fw-semibold countWishlist-{{ $product->id }}">
                ...
                @if ($product['product_type'] == 'digital')
                    <div class="digital-product-authors mb-2">
                        ...
                        @if (count($productAuthorsInfo['data']) > 0)
                            <div class="d-flex align-items-center g-2 me-2">
                                <span class="text-capitalize digital-product-author-title">{{ translate('Author') }}
                                    :</span>
                                <div class="item-list">
                ...
                <form class="addToCartDynamicForm add-to-cart-details-form d-flex flex-column gap-3">
                    ...
                    @php
                        $quickViewProductShapes = json_decode($product->shapes) ?? [];
                    @endphp
                    @if (count($quickViewProductShapes) > 0)
                        <div class="flex-start align-items-center gap-3 mb-2">
                            <div class="product-description-label text-body fs-14 text-nowrap">
                                {{ translate('shape') }}:
                            </div>
                            <div>
                                <ul
                                    class="flex-start checkbox-alphanumeric checkbox-alphanumeric--style-1 mb-0 p-0 list-inline gap-2">
                                    @foreach ($quickViewProductShapes as $key => $shapeId)
                                        @php
                                            $quickViewShape = \App\Models\Shape::find($shapeId);
                                        @endphp
                                        @if ($quickViewShape)
                                            <li>
                                                <input type="radio"
                                                    id="{{ $product->id }}-shape-{{ $shapeId }}"
                                                    name="shape" value="{{ $shapeId }}"
                                                    @if ($key == 0) checked @endif>
                                                <label class="__text-12px quick-view-preview-image-by-shape"
                                                    for="{{ $product->id }}-shape-{{ $shapeId }}"
                                                    data-toggle="tooltip" data-key="{{ $shapeId }}"
                                                    data-title="{{ $quickViewShape->name }}">
                                                    <span class="text-nowrap">{{ $quickViewShape->name }}</span>
                                                </label>
                                            </li>
                                        @endif
                                    @endforeach
                    ...
                    @if (
                        ...
                        @foreach ($product['digital_product_extensions'] as $extensionKey => $extensionGroup)
                            <div class="row flex-start mx-0 align-items-center">
                                <div class="product-description-label text-body fs-14 text-capitalize text-nowrap">
                            ...
                            <div class="d-flex align-items-center gap-3 mb-30">
                                ...
                                <div
                                    ...
                                    <span class="input-group-btn h-100">
                                        <button
                                            class="btn btn-number __p-10 web-text-primary bg-ECF1F6 rounded-0 w-32px"
                                    ...
                                    <span class="input-group-btn h-100">
                                        <button
                                            class="btn btn-number __p-10 web-text-primary bg-ECF1F6 rounded-0 w-32px"
                                            type="button" data-producttype="{{ $product->product_type }}"
                    ...
                    <div class="__btn-grp align-items-center product-add-and-buy-section" {!! $firstVariationQuantity <= 0 ? 'style="display: none;"' : '' !!}>
                        @if (
                        ...
                        @else
                            <button class="btn btn-secondary product-buy-now-button" type="button"
...
<script>
    ...
    // Handle shape selection in quick view
    $(document).on('change', 'input[name="shape"]', function() {
        const selectedShape = $(this).val();
        const shapes = @json(json_decode($product->shapes) ?? []);
        const colorImagesCount = {{ count($product->color_images_full_url ?? $product->images_full_url) }};
        const shapeIndex = shapes.indexOf(selectedShape);

        if (shapeIndex !== -1) {
            const mainSwiper = document.querySelector('.quickviewSlider2').swiper;
            const thumbSwiper = document.querySelector('.quickviewSliderThumb2').swiper;
            // Offset by color images count to get to shape images
            mainSwiper.slideTo(colorImagesCount + shapeIndex);
            thumbSwiper.slideTo(colorImagesCount + shapeIndex);
        }
    });
</script>
