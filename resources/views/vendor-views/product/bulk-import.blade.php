                ...
                <div class="card card-body">
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                <tr class="table-info">
                                    ...
                                    <td>Text</td>
                                    <td><span class="badge badge-warning">No</span></td>
                                    <td>Attribute IDs or Names separated by comma (see attribute list below)</td>
                                    <td>KG,Size or 1,2</td>
                                </tr>
                                <tr class="table-info">
                                    <td><strong>attribute_{name}</strong> or<br><strong>attribute_{id}_values</strong></td>
                                    <td>Text</td>
                                    <td><span class="badge badge-warning">No</span></td>
                                    <td>Values for each attribute (by name or ID), separated by comma. Use attribute name
                                        (e.g., KG) or ID (e.g., 1)</td>
                    ...
                    <div class="alert alert-soft-info mt-3">
                        ...
                        <ul class="mb-0 mt-2">
                            ...
                            <li>For attributes, use the attribute name (e.g., "KG") in the "attributes" column and
                                <code>attribute_KG</code> column for values, or use attribute IDs with
                                <code>attribute_1_values</code> pattern</li>
                            <li>After upload, you need to manually set product images for each color/shape from the product
                                edit page</li>
                        </ul>
                    </div>

                    <div class="alert alert-soft-success mt-3">
                    ...
                    <h3 class="mb-3"><i class="tio-color-bucket text-primary"></i> {{ translate('Available Colors') }}
                    </h3>
                    <div class="alert alert-soft-secondary">
                        <p class="mb-2"><strong>{{ translate('How to use') }}:</strong>
                            {{ translate('Add the color codes separated by comma in the "colors" column') }}</p>
                        <p class="mb-0"><strong>{{ translate('Example') }}:</strong> <code>#FF0000,#00FF00,#0000FF</code>
                        </p>
                    </div>
                                ...
                                @forelse($colors as $color)
                                    <tr>
                                        <td class="text-center">
                                            <span
                                                style="display: inline-block; width: 30px; height: 30px; background-color: {{ $color->code }}; border-radius: 4px; border: 1px solid #ddd;"></span>
                                        </td>
                                        <td>{{ $color->name }}</td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="text-center text-muted">
                                            {{ translate('No colors available') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="alert alert-soft-secondary">
                        <p class="mb-2"><strong>{{ translate('How to use') }}:</strong>
                            {{ translate('Add the shape IDs separated by comma in the "shapes" column') }}</p>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="2" class="text-center text-muted">
                                            {{ translate('No shapes available') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="alert alert-soft-secondary">
                        ...
                        <ol class="mb-0">
                            <li>{{ translate('Add the attribute IDs or Names in the "attributes" column') }}:
                                <code>KG,Size</code> or <code>1,2</code></li>
                            <li>{{ translate('For each attribute, add a column with values using either pattern') }}:</li>
                            <ul class="mt-1 mb-0">
                                <li><strong>{{ translate('By Name') }}:</strong> <code>attribute_KG</code> =
                                    <code>1kg,2kg,3kg</code></li>
                                <li><strong>{{ translate('By ID') }}:</strong> <code>attribute_1_values</code> =
                                    <code>Small,Medium,Large</code></li>
                            </ul>
                            <li>{{ translate('Both patterns work') }}: <code>attribute_{name}</code>,
                                <code>attribute_{name}_values</code>, <code>attribute_{id}</code>, or
                                <code>attribute_{id}_values</code></li>
                        </ol>
                    </div>
                    <div class="table-responsive mb-4" style="max-height: 300px; overflow-y: auto;">
                        <table class="table table-bordered table-sm table-hover">
                            ...
                            <tbody>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="text-center text-muted">
                                            {{ translate('No attributes available') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="alert alert-soft-warning">
                        ...
                        <ul class="mb-0 mt-2">
                            <li>{{ translate('When colors, shapes, or attributes are added, the system will automatically generate variant combinations') }}
                            </li>
                            <li>{{ translate('Each variant will have the same price as unit_price and same stock as current_stock / number of variants') }}
                            </li>
                            <li>{{ translate('You can edit individual variant prices and stock after import from the product edit page') }}
                            </li>
                            <li>{{ translate('Product images for each color/shape need to be added manually after import') }}
                            </li>
                        </ul>
                    </div>
                </div>
