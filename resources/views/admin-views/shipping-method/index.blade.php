        ...
        <div class="card">
            <div class="card-body d-flex flex-column gap-20">
                <div id="order_wise_shipping">
                    {{-- Order Wise Shipping Cost --}}
                    <div class="card card-sm shadow-2 mb-3" id="order_wise_cost_section" 
                        style="{{ $shippingType == 'order_wise' ? '' : 'display:none;' }}">
                        <div class="card-body">
                            <div class="mb-4">
                                <h3>{{ translate('Order_Wise_Shipping_Cost') }}</h3>
                                <p class="mb-0 fs-12">
                                    {{ translate('set_the_shipping_cost_that_applies_to_all_orders_when_order_wise_method_is_enabled.') }}
                                </p>
                            </div>

                            <form action="{{ route('admin.business-settings.order-wise-shipping-cost.update') }}" method="post">
                                @csrf
                                <input type="hidden" name="shippingType" value="order_wise">
                                <div class="px-20 py-4 bg-section rounded mb-20">
                                    <div class="form-group">
                                        <label class="form-label">
                                            {{ translate('Order Wise Shipping Cost') }}
                                            <span class="text-danger">*</span>
                                        </label>

                                        <input
                                            type="number"
                                            name="order_wise_shipping_cost"
                                            min="0"
                                            step="0.01"
                                            class="form-control"
                                            placeholder="{{ translate('enter_order_wise_shipping_cost') }}"
                                            value="{{ getWebConfig('order_wise_shipping_cost')  ?? '' }}"
                                            required
                                        >
                                    </div>
                                </div>
